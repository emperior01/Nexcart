...
import { Plus, Trash2, Save, ImageIcon, Type, Megaphone, Gift, Star, Truck, LayoutGrid, Upload, Link as LinkIcon, GripVertical, DollarSign } from "lucide-react";
...
import { ISO_CURRENCIES } from "@/lib/currency";
...
export default function AdminSettings() {
  ...
  const { setMarketplaceCurrency: syncCurrencyCtx } = useCurrency();
  const [mktCurrency, setMktCurrency] = useState("USD");
  useEffect(() => { if (settings?.marketplace_currency) setMktCurrency(settings.marketplace_currency); }, [settings]);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-3 sm:p-6">
      ...
      {/* ── Currency Management ───────────────────────────────────────────── */}
      <Section icon={DollarSign} title="Currency Management">
        <p className="text-xs text-muted-foreground mb-4">
          Set the default marketplace currency. All prices across the admin dashboard, seller dashboard, product pages, cart, checkout, and orders will use this currency symbol and formatting automatically.
        </p>
        <div className="space-y-3">
          <Label>Default Marketplace Currency</Label>
          <CurrencyPicker
            value={mktCurrency}
            onChange={setMktCurrency}
            currencies={ISO_CURRENCIES.map((c) => ({ ...c, sort_order: 0 }))}
          />
          {mktCurrency && (() => {
            const meta = ISO_CURRENCIES.find(c => c.code === mktCurrency);
            if (!meta) return null;
            let preview = "";
            try {
              preview = new Intl.NumberFormat(meta.locale, { style: "currency", currency: mktCurrency }).format(10000);
            } catch { preview = `${meta.symbol}10,000.00`; }
            return (
              <p className="text-xs text-muted-foreground pt-1">
                Preview: <strong className="text-foreground">{preview}</strong> — {meta.name}
              </p>
            );
          })()}
        </div>
        <Button
          ...
          style={{ background: "linear-gradient(135deg,#E8611A,#C4511A)" }}
          disabled={saving === "marketplace_currency"}
          onClick={async () => {
            await save("marketplace_currency", mktCurrency);
            syncCurrencyCtx(mktCurrency);
          }}
        >
          <Save className="h-4 w-4" /> {saving === "marketplace_currency" ? "Saving…" : "Save Currency"}
        </Button>
      </Section>
    </div>
  );
