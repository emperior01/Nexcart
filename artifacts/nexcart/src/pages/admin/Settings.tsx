...
export default function AdminSettings() {
  ...
  const [heroValidatingIndex, setHeroValidatingIndex] = useState<number | null>(null);
  ...
  async function uploadHeroImage(file: File) {
    ...
    try {
      ...
      setHero((h) => ({ ...h, images: [...h.images, url] }));
  ...
  async function handleHeroUrlBlur(i: number, rawUrl: string) {
    if (!rawUrl.trim()) return;
    setHeroValidatingIndex(i);
    const result = await resolveImageUrl(rawUrl);
    setHeroValidatingIndex(null);
    if (!result.ok) {
                ...
                <div key={i} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <Input
                      ...
                      onBlur={(e) => handleHeroUrlBlur(i, e.target.value)}
                      placeholder={`Image ${i + 1} URL`}
                      disabled={heroValidatingIndex === i}
                    />
                    {heroValidatingIndex === i && (
                      <p className="text-xs mt-1" style={{ color: "#E8611A" }}>Checking image…</p>
                    )}
                  </div>
