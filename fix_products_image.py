#!/usr/bin/env python3
"""
Rewrite seller/Products.tsx:
- Upload image from device (uploaded immediately on file select, not deferred)
- OR paste an image URL
- Both work independently; whichever is filled last wins
Run from ~/nexcart_scott:
    python3 fix_products_image.py
"""
import os

BASE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "artifacts/nexcart")
path = os.path.join(BASE, "src/pages/seller/Products.tsx")
...
print(f"✅ Written: artifacts/nexcart/src/pages/seller/Products.tsx")
print("""
Make sure the Supabase Storage bucket + policies exist.
...
Then push:
  cd ~/nexcart_scott
  git add artifacts/nexcart/src/pages/seller/Products.tsx
  git commit -m "feat: seller product image — upload from device + paste URL"
  git push
""")
